...
const SellerProductDetails = () => {
  ...
  const handleAddNewVariant = async () => {
    ...
    if (!hasValidAttribute) { alert("At least one valid attribute is required."); return; }
...
};
{
  ...
  const handleAttributeChange = (index, field, value) => {
    ...
    const newAttrsObj = {};
    updatedInputs.forEach(attr => { if (attr.key.trim() !== '') newAttrsObj[ attr.key.trim() ] = attr.value; });
  ...
  const handleImageUpload = (e) => {
    ...
    if (files.length > availableSlots) alert(`You can only upload up to 7 images. ${filesToAdd.length} added.`);
  ...
  if (loading) return (
    <>
      ...
      <div className="min-h-screen bg-[#fbf9f6] flex items-center justify-center text-[#1b1c1a] font-serif">Loading gallery...</div>
    </>
  );
  ...
  if (!product) return (
    <>
      <style>{`*{scrollbar-width:none!important;-ms-overflow-style:none!important;}*::-webkit-scrollbar{display:none!important;}`}</style>
      <div className="min-h-screen bg-[#fbf9f6] flex items-center justify-center text-[#1b1c1a] font-serif">Product Not Found</div>
    </>
  );
